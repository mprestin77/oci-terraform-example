...
resource "oci_core_vcn" "vcn" {
  ...
  cidr_block     = var.network_params.vcn_cidr
  compartment_id = var.compartment_ocid
  display_name   = var.network_params.vcn_name
  dns_label      = var.network_params.vcn_dns_label
}
...
resource "oci_core_subnet" "public_subnet" {
  count                      = var.use_existing_vcn ? 0 : 1
  cidr_block                 = var.network_params.public_subnet_cidr
...
resource "oci_core_subnet" "private_subnet" {
  count                      = var.use_existing_vcn ? 0 : 1
  cidr_block                 = var.network_params.private_subnet_cidr
